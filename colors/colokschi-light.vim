...
hi CursorColumn		ctermfg=NONE 	ctermbg=252 	cterm=NONE	" The screen column that the cursor is in when 'cursorcolumn' is set
hi CursorLine		ctermfg=NONE 	ctermbg=NONE	cterm=REVERSE	" The screen line that the cursor is in when 'cursorline' is set
hi CursorLineNr		ctermfg=202	ctermbg=231	cterm=BOLD 	" Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line
...
hi EndOfBuffer		ctermfg=231	ctermbg=231 	cterm=NONE	" Filler lines (~) after the last line in the buffer. By default, this is highlighted like hl-NonText
hi ErrorMsg		ctermfg=231	ctermbg=NONE	cterm=BOLD	" Error messages on the command line
hi Folded		ctermfg=252	ctermbg=244	cterm=NONE 	" Line used for closed folds
hi FoldColumn		ctermfg=208	ctermbg=244	cterm=BOLD 	" 'foldcolumn'
hi IncSearch		ctermfg=250	ctermbg=27	cterm=NONE 	" 'incsearch' highlighting; also used for the text replaced with ":s///c"
hi LineNr		ctermfg=231	ctermbg=247	cterm=NONE 	" Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set
...
hi NonText		ctermfg=241	ctermbg=NONE	cterm=NONE 	" '@' at the end of the window, characters from 'showbreak' and other characters that do not really exist in the text (e.g., ">" displayed when a double-wide character doesn't fit at the end of the line).
hi Normal		ctermfg=241	ctermbg=231 	cterm=NONE 	" Normal text
...
hi SignColumn		ctermfg=NONE 	ctermbg=242 	cterm=NONE 	" Column where signs are displayed
hi SpecialKey		ctermfg=247	ctermbg=NONE	cterm=NONE 	" Meta and special keys listed with ":map", also for text used to show unprintable characters in the text, 'listchars'. Generally: text that is displayed differently from what it really is.
...
hi Comment		ctermfg=250	ctermbg=NONE	cterm=NONE	" Any comment
